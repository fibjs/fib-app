...
exports.bind = (_, app) => {
    ...
    api.rfind = (req, db, cls, id, relation) => {
        ...
        if (rel_model.type === 'hasOne') {
            ...
            var rid = obj.data[Object.keys(obj.data.__opts.one_associations.find(a => a.name === relation).field)[0]];
            var robj = _get(rel_model.model, rid, req.session, "+read");
            if (robj.error)
                return robj;
            ...
            var _association = obj.data.__opts.many_associations.find(a => a.name === relation);
            return {
