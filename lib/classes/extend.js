...
exports.bind = (_, app) => {
    ...
    api.rput = (req, db, cls, id, extend, rid, data) => {
        ...
        var obj = _get(cls, null, 'find', id, req.session, "write", extend);
        if (obj.error)
            return obj;
        ...
        if (data == undefined) {
            ...
            if (rel_model.type === 'hasOne') {
                if (rel_model.reversed) {
                    var robj = _get(rel_model.model, obj.data, obj.data.__opts.one_associations.find(a => a.name === extend).getAccessor, rid, req.session, "+write");
                    if (robj.error)
                        return robj;
                } else {
                ...
                var robj = _get(rel_model.model, obj.data, obj.data.__opts.many_associations.find(a => a.name === extend).getAccessor, rid, req.session, "+write");
                if (robj.error)
                    return robj;
            }
    ...
    api.rfind = (req, db, cls, id, extend) => {
        ...
        var obj = _get(cls, null, 'find', id, req.session, "read", extend);
        if (obj.error)
            return obj;
    ...
    api.rget = (req, db, cls, id, extend, rid) => {
        ...
        if (rel_model.type === 'hasOne') {
            if (rel_model.reversed) {
                var robj = _get(rel_model.model, obj.data, obj.data.__opts.one_associations.find(a => a.name === extend).getAccessor, rid, req.session, "+read");
                if (robj.error)
                    return robj;
            } else {
                if (rid != obj.data[Object.keys(obj.data.__opts.one_associations.find(a => a.name === extend).field)[0]])
            ...
            var robj = _get(rel_model.model, obj.data, obj.data.__opts.many_associations.find(a => a.name === extend).getAccessor, rid, req.session, "+read");
            if (robj.error)
                return robj;
        }
    ...
    app.put('/:classname/:id/:extend', (req, classname, id, extend) => _(req, classname, id, extend, api.rput));
