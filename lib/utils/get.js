...
exports._get = function (cls, id, session, act) {
    ...
    if (act) {
        var acl = check_acl(session, act, obj.data.ACL);
        if (acl === undefined)
            acl = check_acl(session, act, cls.ACL);
        if (!acl)
...
exports._eget1 = function (cls, id, extend, rid, session, act) {
    ...
    if (act) {
        var acl = check_acl(session, act, robj.data.ACL);
        if (acl === undefined)
            acl = check_acl(session, act, obj.data.ACL, extend);
        if (acl === undefined)
            acl = check_acl(session, act, cls.ACL, extend);
        if (acl === undefined)
            acl = check_acl(session, act, rel_model.model.ACL);
        if (!acl)
            return err_info(4030001, {}, cls.cid);
        robj.acl = acl;
    }
